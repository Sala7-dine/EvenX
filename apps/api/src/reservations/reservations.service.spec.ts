...
describe('ReservationsService', () => {
    ...
    class MockReservationModel {
        ...
        save = jest.fn().mockImplementation(() => Promise.resolve({ ...this.data, _id: 'resId' }));
        static find = jest.fn();
    ...
    describe('create', () => {
        ...
        it('should throw NotFoundException if event does not exist', async () => {
            ...
            await expect(service.create({ eventId: 'invalid' }, 'user')).rejects.toThrow(NotFoundException);
        });
        ...
        it('should throw BadRequestException if event is full', async () => {
            ...
            await expect(service.create({ eventId: 'eventId' }, 'user')).rejects.toThrow(BadRequestException);
        });
    });

    describe('confirm', () => {
        it('should confirm reservation', async () => {
            const res = { ...mockReservation, status: ReservationStatus.PENDING, save: jest.fn().mockResolvedValue({ ...mockReservation, status: ReservationStatus.CONFIRMED }) };
    ...
    describe('cancel', () => {
        ...
        it('should forbid participant cancelling others reservation', async () => {
            const res = { ...mockReservation, userId: 'otherUser', toString: () => 'otherUser' };
            ...
            jest.spyOn(MockReservationModel, 'findById').mockResolvedValue(mockResObj);

            await expect(service.cancel('resId', 'myUser')).rejects.toThrow(ForbiddenException);
        });
    });
});
